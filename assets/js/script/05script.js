...
const ACC1 = '#00d0ff';   // neon blue
const ACC2 = '#7fffd4';   // aqua / mint
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  container.innerHTML = "";
  industryData.timelineGroups.forEach(group => {
  ...
  function openModal(m) {
    ...
    // ensure canvases render after layout
  ...
  // ==== Global reach map (abstract arcs) ====
  function drawMap(c, regions) {
    ...
    const pts = {
      Iran: [w * 0.45, h * 0.55],
      Europe: [w * 0.25, h * 0.35],
      USA: [w * 0.12, h * 0.40],
      "Middle East": [w * 0.55, h * 0.55],
      Global: [w * 0.35, h * 0.25]
    };
