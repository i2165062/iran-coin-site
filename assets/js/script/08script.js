...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // Reveal animation without flicker
  ...
  function drawTimeline(c, m){
    ...
    // keep in frame even if span is short
  ...
  function drawMap(c, regions){
    ...
    const pts = {
      Iran:[w*0.42,h*0.55], Anatolia:[w*0.48,h*0.40], Europe:[w*0.55,h*0.30],
      India:[w*0.56,h*0.62], CentralAsia:[w*0.50,h*0.45], USA:[w*0.22,h*0.35],
      Caucasus:[w*0.48,h*0.36], SouthAsia:[w*0.60,h*0.64]
    };
    ...
    (regions||[]).forEach(r=>{
      const key = r.replace(/\s+/g,''); // map keys without spaces
      if(!pts[key]) return;
      const [tx, ty] = pts[key];
